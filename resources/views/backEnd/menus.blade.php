                            ...
                            <table class="table table-striped  b-t">
                                ...
                                @foreach($Menus as $Menu)
                                    ...
                                    @foreach($Menu->subMenus as $subMenu)
                                        ...
                                        @foreach($subMenu->subMenus as $subMenun)
                                        <?php
                                        if ($subMenun->$title_var != "") {
                                            $titlen= $subMenun->$title_var;
                                        } else {
                                            $titlen = $subMenun->$title_var2;
                                        }
                                        ?>
                                        <tr>
                                            <td><label class="ui-check m-a-0">
                                                    <input type="checkbox" name="ids[]" value="{{ $subMenun->id }}"><i
                                                            class="dark-white"></i>
                                                    {!! Form::hidden('row_ids[]',$subMenun->id, array('class' => 'form-control row_no')) !!}
                                                </label>
                                            </td>
                                            <td>
                                                <img src="{{ URL::to('plugins/backEnd/assets/images/treepart_'.trans('backLang.direction').'.png') }}" class="submenu_tree">
                                                ...
                                                {!! $titlen  !!}</td>
                                            <td class="text-center">
                                                <i class="fa {{ ($subMenun->status==1) ? "fa-check text-success":"fa-times text-danger" }} inline"></i>
                                            </td>
                                            <td class="text-center">
                                                <a class="btn btn-sm success"
                                                   href="{{ route("menusEdit",["ParentMenuId"=>$edt_id,"id"=>$subMenun->id]) }}">
                                                    <small><i class="material-icons">
                                                            &#xe3c9;</i> {{ trans('backLang.edit') }}
                                                    </small>
                                                </a>
