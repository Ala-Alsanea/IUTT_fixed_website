...
<style type="text/css">
 ...
 $universitycenters=App\Models\Topic::where([['status', 1],['webmaster_id',$WebmasterSectioncenters->id]])->orderby('row_no', 'asc')->get();
            ...
            ?>
               ...
               {{ ($WebmasterSection->name=='departments')?'required':'' }} 
        ...
        ?>
          @if($WebmasterSection->name=='news')
           ...
           @else
            ...
            @endif
            <optgroup label="{{ $titleLable }}"><span></span>
      ...
      <optgroup label="{{ trans('backLang.universitycenters') }}"><span></span>
        @foreach ($universitycenters as $Item)
            <?php
            if ($Item->$title_var != "") {
                $ftitle = $Item->$title_var;
            } else {
                $ftitle = $Item->$title_var2;
            }
            ?>
               <option value="{{ $Item->id  }}" {{ ($Item->id==$Topics->father_id) ? "selected='selected'":""  }}>{{ $ftitle }}</option>
...
@endif
 ...
 $sitePages=App\Models\Topic::where([['status', 1],['webmaster_id',1]])->orderby('row_no', 'asc')->get();
            ...
            ?>
               ...
               <select name="refrence_id"  id="refrence_id"  class="form-control"  ui-jp="select2"    placeholder="{{ trans('backLang.father_id') }}"
                                    ui-options="{theme: 'bootstrap'}">
        <?php
        $title_var = "title_" . trans('backLang.boxCode');
        $title_var2 = "title_" . trans('backLang.boxCodeOther');
        ...
        @foreach ($sitePages as $sitePage)
            ...
            ?>
               <option value="{{ $sitePage->id  }}" {{ ($sitePage->id==$Topics->refrence_id) ? "selected='selected'":""  }}>{{ $sitePagetitle }}</option>
