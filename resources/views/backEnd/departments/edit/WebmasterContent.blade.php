 ...
 $faculties=App\Models\Faculty::where('status', 1)->orderby('row_no', 'asc')->get();
        ...
        @endforeach
         ...
         <div class="form-group row">
        <label for="type_program"
               class="col-sm-2 form-control-label">{!!  trans('backLang.type_program') !!} </label>
        <div class="col-sm-10"> 
          
 <select name="type_program" id="type_program" class="form-control " required
                                     placeholder="{{ trans('backLang.type_program')  }}"
                                    ui-options="{theme: 'bootstrap'}"> 
            <option value="0"></option>
            <option value="1" {{ ($Departments->type_program==1)?'selected':'' }}>{{ trans('frontLang.undergraduate_programs')  }}</option>
         
         <option value="2" {{ ($Departments->type_program==2)?'selected':'' }}>{{ trans('frontLang.graduate_programs')  }}</option>    
    </select>
