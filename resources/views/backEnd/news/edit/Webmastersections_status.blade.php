  @if($WebmasterSection->sections_status!=0)
    <div class="form-group row">
        ...
        <div class="col-sm-10">
          {{--   <select  {{ ()?'name="section_id[]"':'name="section_id"' }}     id="section_id" class="form-control select2-multiple"
                    {{ ($WebmasterSection->id==3)?'multiple':'' }} 
                    ui-jp="select2"
                    ui-options="{theme: 'bootstrap'}" > --}}
                 @if($WebmasterSection->id==3 || $WebmasterSection->name=='academicstaff')
