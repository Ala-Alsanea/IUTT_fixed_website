 ...
 $faculties=App\Models\Faculty::where('status', 1)->orderby('row_no', 'asc')->get();
   ...
   <div class="form-group row">
        <label for="father_id"
               class="col-sm-2 form-control-label">{!!  trans('backLang.programs') !!}  </label>
        <div class="col-sm-10">
            <?php 
         //   ['webmasterSection.name','faculties']
            $title_var = "title_" . trans('backLang.boxCode');
 $programs=App\Models\Program::where('status', 1)->orderby('row_no', 'asc')->get();
         
            ?>
          
               <select name="father_id" id="father_id" class="form-control "
              required 
                                    ui-jp="select2" placeholder="{!!  trans('backLang.programs') !!} "
                                    ui-options="{theme: 'bootstrap'}">
        <?php
        $title_var = "title_" . trans('backLang.boxCode');
        $title_var2 = "title_" . trans('backLang.boxCodeOther');
        ...
        @foreach ($programs as $program)
            <?php
            if ($program->$title_var != "") {
                $ftitle = $program->$title_var;
            } else {
                $ftitle = $program->$title_var2;
            }
            ?>
               <option value="{{ $program->id  }}" {{ ($program->id==$contentsections->father_id) ? "selected='selected'":""  }}>{{ $ftitle }}</option>
...
@if(Helper::GeneralWebmasterSettings("ar_box_status"))
 ...
 <div class="form-group hidden row">
    ...
    <div class="col-sm-10">
        ...
        {!! Form::text('banner',$contentsections->banner, array('placeholder' => '','class' => 'form-control','id'=>'banner','required'=>'')) !!}
 ...
 <select name="catagoryes" id="catagoryes" class="form-control " required
            ...
            <option value="0"></option>
 <option value="1" {{ ($contentsections->catagoryes==1)?'selected':'' }}>{{ trans('frontLang.coursedescriptions')  }}</option>
 <option value="2"  {{ ($contentsections->catagoryes==2)?'selected':'' }}>{{ trans('frontLang.scholarships')  }}</option>
