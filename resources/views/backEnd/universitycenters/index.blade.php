    ...
    <div class="padding">
        <div class="box">
            <div class="box-header dker">
                <h3>{!! trans('backLang.universitycenters') !!}</h3>
                <small>
                    <a href="{{ route('adminHome') }}">{{ trans('backLang.home') }}</a> /
                    <a>{!! trans('backLang.universitycenters') !!}</a>
                </small>
            </div>
            @if($universitycenters->total() >0)
                @if(@Auth::user()->permissionsGroup->add_status)
                    <div class="row p-a">
                        <div class="col-sm-12">
                        <a class="btn btn-fw primary" href="{{route("universitycentersCreate")}}">
                                <i class="material-icons">&#xe02e;</i>
                                &nbsp; {{ trans('backLang.topicNew') }}  {!! trans('backLang.universitycenters') !!}
                            </a>
                        </div>
                    </div>
                @endif
           ...
           <?php 
                                ...
                                ?>
            @if($universitycenters->total() > 0)
                {{Form::open(['route'=>['universitycentersUpdateAll'],'method'=>'post'])}}
                <div class="table-responsive">
                    <table class="table table-striped  b-t">
                        ...
                        ?>
                        @foreach($universitycenters as $Topic)
                                ...
                                <td class="text-center">
                                    @if(@Auth::user()->permissionsGroup->edit_status)
                                        <a class="btn btn-sm success"
                                           href="{{ route("universitycentersEdit",["id"=>$Topic->id]) }}">
                                ...
                                <div class="modal-dialog" id="animate">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <a href="{{ route("universitycentersDestroy",["id"=>$Topic->id]) }}"
                ...
                <footer class="dker p-a">
                    <div class="row">
                        ...
                        <div class="col-sm-3 text-center">
                            <small class="text-muted inline m-t-sm m-b-sm">{{ trans('backLang.showing') }} {{ $universitycenters->firstItem() }}
                                -{{ $universitycenters->lastItem() }} {{ trans('backLang.of') }}
                                <strong>{{ $universitycenters->total()  }}</strong> {{ trans('backLang.records') }}</small>
                        </div>
                        <div class="col-sm-6 text-right text-center-xs">
                            {!! $universitycenters->links() !!}
                        </div>
                    </div>
                </footer>
                {{Form::close()}}
