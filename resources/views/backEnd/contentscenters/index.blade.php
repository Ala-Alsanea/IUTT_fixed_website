    ...
    <div class="padding">
        <div class="box">
            <div class="box-header dker">
                <h3>{!! trans('backLang.contentscenters') !!}</h3>
                <small>
                    <a href="{{ route('adminHome') }}">{{ trans('backLang.home') }}</a> /
                    <a>{!! trans('backLang.contentscenters') !!}</a>
                </small>
            </div>
            @if($contentsections->total() >0)
                @if(@Auth::user()->permissionsGroup->add_status)
                    <div class="row p-a">
                        <div class="col-sm-12">
                        <a class="btn btn-fw primary" href="{{route("contentscentersCreate")}}">
                                <i class="material-icons">&#xe02e;</i>
                                &nbsp; {{ trans('backLang.topicNew') }}  {!! trans('backLang.contentscenters') !!}
                            </a>
                        </div>
                    </div>
                @endif
           ...
           <?php 
                                ...
                                ?>
            @if($contentsections->total() > 0)
                {{Form::open(['route'=>['contentscentersUpdateAll'],'method'=>'post'])}}
                    ...
                    <table class="table table-striped  b-t">
                        ...
                        <tr>
                            ...
                            <th>{{ trans('backLang.topicName') }}</th>
                               ...
                               <th>{{ trans('backLang.universitycenters') }}</th>
                        ...
                        ?>
                        @foreach($contentsections as $Topic)
                            ...
                            {{--    <td class="text-center">
                                  @if(count((array)$Topic->faculty)>0 && isset( $Topic->faculty->$title_var))
                                    {!!   $Topic->faculty->$title_var !!}
                                   @endif
                                </td> --}}
                                 ...
                                 <td class="text-center">
                                  ...
                                  <?php 
                        $contentscenters=App\Models\UniversityCenter::find($Topic->father_id);
                                if (count((array)$contentscenters)>0) {
                                     echo $contentscenters->$title_var;
                                }
                                  ?>
                                ...
                                <td class="text-center">
                                    @if(@Auth::user()->permissionsGroup->edit_status)
                                        <a class="btn btn-sm success"
                                           href="{{ route("contentscentersEdit",["id"=>$Topic->id]) }}">
                                ...
                                <div class="modal-dialog" id="animate">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <a href="{{ route("contentscentersDestroy",["id"=>$Topic->id]) }}"
                ...
                <footer class="dker p-a">
                    <div class="row">
                        ...
                        <div class="col-sm-3 text-center">
                            <small class="text-muted inline m-t-sm m-b-sm">{{ trans('backLang.showing') }} {{ $contentsections->firstItem() }}
                                -{{ $contentsections->lastItem() }} {{ trans('backLang.of') }}
                                <strong>{{ $contentsections->total()  }}</strong> {{ trans('backLang.records') }}</small>
                        </div>
