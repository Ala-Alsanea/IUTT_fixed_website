 ...
 $universitycenters=App\Models\UniversityCenter::where('status', 1)->orderby('row_no', 'asc')->get();
            ...
            ?>
              ...
              required 
                                    ui-jp="select2" placeholder="{!!  trans('backLang.universitycenters') !!} "
                                    ui-options="{theme: 'bootstrap'}">
        <?php
        $title_var = "title_" . trans('backLang.boxCode');
        $title_var2 = "title_" . trans('backLang.boxCodeOther');
        ...
        @foreach ($universitycenters as $universitycenter)
            ...
            ?>
               <option value="{{ $universitycenter->id  }}" {{ ($universitycenter->id==$contentsections->father_id) ? "selected='selected'":""  }}>{{ $ftitle }}</option>
