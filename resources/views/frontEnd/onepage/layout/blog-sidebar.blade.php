...
$pullleft='right'; 
    ...
    <div class="widget sidebar_widget widget_search col-lg-12 col-md-12 col-xs-12 col-sm-12">
         {{Form::open(['route'=>['searchTopicsFaculty',$FacultyData->$title_var],'method'=>'POST','class'=>'search-form input-group'])}}
            <div class="input-group input-group-sm">
                {!! Form::text('search_word',@$search_word, array('placeholder' => trans('frontLang.search'),'class' => 'form-control','id'=>'search_word','required'=>'','type'=>'search')) !!}
    ...
    @if(count((array)$TopicsMostViewed)>0)
      <?php
            ...
            ?>
    <div class="widget sidebar_widget widget_recent_post mt_60 col-lg-12 col-md-8 col-xs-12 col-sm-12">
        <div class="widget_title">
          ...
          @foreach($TopicsMostViewed as $TopicMostViewed)
           <?php
             ...
             $topic_link_url = url(trans('backLang.code').'/news/faculty/'.$TopicMostViewed->id);
                          $topic_link_url = url(trans('backLang.code').'/'.$FacultyData->id.'/news/faculty/'.$TopicMostViewed->id);   
     ...
     @endif
       @if(count((array)$Categories)>0)
        <?php
            ...
            $slug_var2 = "seo_url_slug_" . trans('backLang.boxCodeOther');
            ?>
    <div class="widget sidebar_widget widget_categorie mt_60 col-lg-12 col-md-4 col-xs-12 col-sm-12 techsoft">
        <div class="widget_title">
             ...
             @foreach($Categories as $Category)
                       ...
                       // count($category_and_topics_count);
                        ...
                        <li>
                            ...
                            @endif
                            <a {{ $active_cat }} href="{{  url(trans('backLang.code').'/'.$FacultyData->id.'/news/faculty/cat/'.$Category->id) }}">{{$Category->$category_title_var}}</a><span
                  ...
                  $SubCategory_link_url = url(trans('backLang.code').'/news/faculty/cat/'.$MnuCategory->id);   
                            ?>
                            <li> &nbsp; &nbsp; &nbsp;
                                @if($MnuCategory->icon !=="")
